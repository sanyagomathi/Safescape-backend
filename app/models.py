...
class Segment(Base):
    """
    Street segment (MVP): a point + optional objective signals (0..1)
    Later you can store geometry / polyline / graph edges.
    """
    __tablename__ = "segments"

    id: Mapped[str] = mapped_column(String(64), primary_key=True)  # e.g. "seg_001"
    lat: Mapped[float] = mapped_column(Float, index=True)
    lng: Mapped[float] = mapped_column(Float, index=True)

    lighting: Mapped[float] = mapped_column(Float, default=0.5)     # 0..1
    crowd: Mapped[float] = mapped_column(Float, default=0.5)        # 0..1
    shops_open: Mapped[float] = mapped_column(Float, default=0.5)   # 0..1
    transport: Mapped[float] = mapped_column(Float, default=0.5)    # 0..1
...
class Score(Base):
    """
    Cached score per (segment_id, hour) for fast map loading.
    """
    __tablename__ = "scores"
    __table_args__ = (UniqueConstraint("segment_id", "hour", name="uq_score_segment_hour"),)
